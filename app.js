// 1. DATABASE INTERNO IMMAGINI PEZZI
const pezziBase64 = {
...
window.cambiaTurno = function(colore) {
    ...
    if (colore === 'w') {
        ...
        $('#btn-black').css({ opacity: '1', borderColor: '#007aff' });
...
function updateSavedList() {
    ...
    savedPositions.forEach(function(item, index) {
        ...
        var $delBtn = $('<button class="btn-sm btn-danger">Elimina</button>').on('click', function() {
            var saved = JSON.parse(localStorage.getItem('chess_positions')) || [];
            saved.splice(index, 1);
            localStorage.setItem('chess_positions', JSON.stringify(saved));
            updateSavedList();
        });
